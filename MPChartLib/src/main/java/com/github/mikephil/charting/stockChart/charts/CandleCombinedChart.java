...
public class CandleCombinedChart extends CombinedChart {
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            KLineDataModel data=kLineData.getKLineDatas().get((int) mIndicesToHighlight[i].getX());
            if (pos[0] >= CommonUtil.getWindowWidth(getContext()) / 2) {

                float yValForXIndex1 = (float) data.getClose();
                myMarkerViewLeft.setNumber(yValForXIndex1);
                myMarkerViewLeft.setData(data);
                ...
                if (getAxisLeft().getLabelPosition() == YAxis.YAxisLabelPosition.OUTSIDE_CHART) {
                    myMarkerViewLeft.draw(canvas, mViewPortHandler.contentLeft() - myMarkerViewLeft.getWidth() / 2, y);//+ CommonUtil.dip2px(getContext(),20)   - myMarkerViewLeft.getHeight() / 2
                } else {
                    myMarkerViewLeft.draw(canvas, mViewPortHandler.contentLeft() + myMarkerViewLeft.getWidth() / 2, y);//+ CommonUtil.dip2px(getContext(),20)   - myMarkerViewLeft.getHeight() / 2
                }
            } else {
                float yValForXIndex2 = (float) data.getClose();
                myMarkerViewRight.setNumber(yValForXIndex2);
                myMarkerViewRight.setData(data);
                ...
                if (getAxisRight().getLabelPosition() == YAxis.YAxisLabelPosition.OUTSIDE_CHART) {
                    ...
                    myMarkerViewRight.draw(canvas, mViewPortHandler.contentRight() + myMarkerViewRight.getWidth() / 2,y );// - CommonUtil.dip2px(getContext(),20)
                } else {
                    myMarkerViewRight.draw(canvas, mViewPortHandler.contentRight() - myMarkerViewRight.getWidth() / 2, y);// - CommonUtil.dip2px(getContext(),20)
                }
            }
